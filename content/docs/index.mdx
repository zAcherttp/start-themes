...
The `useTheme` hook is a lightweight theme management solution designed for TanStack Start applications.

It uses a blocking inline script to prevent [Flash of Unstyled Content (FOUC)](https://en.wikipedia.org/wiki/Flash_of_unstyled_content), fully supports multiple theme presets (colors), and leverages the native View Transitions API for buttery smooth cross-fades.
